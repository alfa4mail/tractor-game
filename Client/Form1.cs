...
namespace WindowsApplication2
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private Boolean GoGetInfo = false;
		...
		private void InitializeComponent()
		{
			...
			this.menuItemParam.MenuItems.AddRange(new System.Windows.Forms.MenuItem[] {
																						  this.menuItemNet});
			this.menuItemParam.Text = "Настройки";
			// 
			// menuItemNet
			// 
			this.menuItemNet.Index = 0;
		...
		private void OnTick(object sender, System.EventArgs e)
		{
			if (GoGetInfo == true)
			{
				...
				GoGetInfo = false;
				backgroundConnection = new Thread(new ThreadStart(this.GetInfo));
		...
		private void GetInfo()
		{
			...
			if (trac2.BombNowCreate == true)
			{
				trac2.BombNowCreate = false;
				command = "Bomb";
			}
			else
				if (PlayerNowMoving == true)
			{
			...
			// ходим первым игроком
			OtherTracMove(result);
		...
		private void OtherTracMove(String str)//!!!
		{
			...
			if (trac.Moving(str, soilList, stoneList,bombList) == true)
			{
				switch (str)
				{
					case "Left": 
		...
		private void OnKeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
		{
			if ((IsCreateNewMap == false) && (IfPlayerMoving == true))
			{
				...
				if (trac2.Moving(e, soilList, stoneList,bombList))
				{
		...
		private void On_Open(object sender, System.EventArgs e)
		{
			...
			if (Open.ShowDialog() == DialogResult.OK)
			{
				...
				if (myStream != null)
				{
					...
					GoGetInfo = true; //Нужно для клиента
